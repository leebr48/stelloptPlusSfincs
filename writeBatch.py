...
with open('batch_email.txt', 'r') as f:
    read = f.readlines()
    if len(read) > 1:
        raise IOError('There was more than one line in the provided batch_email.txt file. Please include only a single email address.')

email = read[0].strip()

# Load the location of the SFINCS directory
with open('sfincs_location.txt', 'r') as f:
    read = f.readlines()
    if len(read) > 1:
        raise IOError('There was more than one line in the provided sfincs_location.txt file. Please include only a single address.')

sfincsLoc = os.path.abspath(read[0].strip()) + '/fortran/version3/sfincs'
