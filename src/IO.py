...
def getChooseErArgs():
    ...
    parser.add_argument('--maxRootJr', type=float, nargs=1, required=False, default=[1.0e-12], help='Maximum radial current (defined as in SFINCS) that may be present for a given electric field value to be considered a "root". The default is the same as that used in writeNamelist.py and is recommended. Note that setting this too low may make it impossible to find any satisfactory roots.')
    args = parser.parse_args()
