...
def getBoozerInformation(inFile, desired_s_vals, write_output=None, booz_toroidal_harmonics=51, booz_poloidal_harmonics=51, verbosity=1, stellSym=True):
    ...
    outDic = {}
    for desired_s_val, closest_s, closest_s_id, B00, R00, G, I, iota in zip(desired_s_vals, closest_ss, closest_s_ids, B00s, R00s, Gs, Is, iotas):
        outDic[desired_s_val] = {'actual_s':closest_s, 'actual_s_id':closest_s_id, 'B00':B00, 'R00':R00, 'G':G, 'I':I, 'iota':iota}
...
def chandraG(x):
    ...
    '''
    Inputs:
        A real number >= 0, or numpy.inf.
    Outputs:
        A real number; the output of the "Chandrasekhar function" G(x).
